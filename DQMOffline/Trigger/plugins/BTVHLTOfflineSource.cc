    ...
    if ((v.getTriggerType() == "PF" && shallowTagInfosPf.isValid()) ||
      ...
      for (const auto& shallowTagInfo : *shallowTagInfoCollection) {
        ...
        for (const auto& tagVar : tagVars.getList(reco::btau::jetNSecondaryVertices, false)) {
          //std::cout<<reco::btau::jetNSecondaryVertices<<endl;
          //std::cout<<"tagged "<<tagVar<<" "<<tagVars.getList(reco::btau::jetNSecondaryVertices, false)[0]<<endl;
          //i++;
...
void BTVHLTOfflineSource::bookHistograms(DQMStore::IBooker& iBooker, edm::Run const& run, edm::EventSetup const& c) {
  ...
  for (auto& v : hltPathsAll_) {
    ...
    ///histoname = "trackSip3dSig";
    ///title = "trackSip3dSig" + trigPath;
    ///v.h_trackSip3dSig = iBooker.book1D(histoname.c_str(), title.c_str(), 60, 0, 60);
    ...
    //histoname = "vertexNTracks";
    //title = "vertexNTracks" + trigPath;
    //v.h_vertexNTracks = iBooker.book1D(histoname.c_str(), title.c_str(), 31, 0, 30);
